...
return function(opt)
    ...
    local function create_agent()
        ...
        if opt.model_one_hot == 1 then
            model_state:add(nn.AddConstant(1))
            model_state:add(nn.View(-1))
            model_state:add(nn.OneHot(9))
            model_state:add(nn.View(-1, 9 * opt.model_input_size))
            model_state:add(nn.Linear( 9 * opt.model_input_size, opt.model_rnn_size))
            model_state:add(nn.ReLU())
        else
            model_state:add(nn.Linear(opt.model_input_size, opt.model_rnn_size))
        end
